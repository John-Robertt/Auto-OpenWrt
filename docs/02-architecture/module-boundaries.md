...
Build Application / Pipeline -> Config/Profile Resolver
...
| Workspace Store | 管理工作区目录、原子写入、run record、manifest、lock、artifact index | 判断 target 或插件兼容性 |
| Config/Profile Resolver | 解析 user config，生成 resolved config，展开默认值 | 修改源码或采纳 AI 补丁 |
| Health Checker | 执行预检和构建上下文校验，生成 Health Report | 代替构建失败诊断 |
| Source Manager | 更新共享源码缓存，创建运行工作树，应用同 profile adopted patches | 决定是否重试构建 |
...
- 工作区是持久状态边界，所有构建记录、产物、诊断、运行工作树、检查点和 adopted patches 必须归属到工作区。
- `sources/` 是共享源码缓存，不是构建时的可变源码区。
- `worktrees/<profile>/<run-id>/` 是当前 run 的唯一可变源码区。
- Build Profile 是多 target 的隔离边界，不同 profile 的产物、日志、运行工作树和 adopted patches 不得混淆。
...
- CLI 到流水线的输入必须是命令参数和工作区定位信息，不传递未解析的全局状态。
- 流水线到模块的输入必须是 resolved config、run id、profile、路径和阶段上下文。
