...
建立 Go CLI、配置解析、project root 初始化、基础健康检查和文档约束，让后续构建流水线有稳定输入。
...
- Workspace Store：创建并校验 project root 与 workspace 状态目录结构。
- YAML user config 解析和 resolved config 输出。
- Run id 生成、doctor health report 写入。
- Run Record 基础状态写入和中断恢复标记。
- 基础 Health Report，覆盖 Docker、权限、project root、workspace 目录、磁盘、网络、配置和 AI CLI 可用性。
...
- `auto-openwrt init` 可以创建 project root 骨架和示例 workspace 状态目录。
...
- 失败项包含原因和建议。
- 初始化后的目录模型能支撑多配置、多 build 和 run 级工作树隔离。
