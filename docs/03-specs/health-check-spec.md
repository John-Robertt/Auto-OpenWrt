...
`doctor` 不要求创建 build profile 的运行工作树；如果用户指定 `--profile`，可以读取配置并检查 profile 声明完整性，但不更新共享源码缓存。

参数错误、配置文件无法读取、YAML 语法错误或配置 schema 错误返回 CLI 退出码 `2`。只有 Health Report 已成功生成且存在阻断项时，才返回退出码 `3`。
...
- `workspace.read_write`：工作区是否可读写。
- `workspace.directories`：必需目录是否存在或可创建。
...
- `config.profile_refs`：profile 引用的 feeds/plugins 是否存在且 enabled。
...
- `docker.mount_scope`：Docker 只挂载当前 run 工作树、缓存、artifact staging 和必要只读 source cache。
...
- profile 引用不存在或 disabled feed/plugin 时阻断构建。
...
- `schema_version`。
- `run_id`。
- `profile`，doctor 未指定 profile 时为 `null`。
- `workspace`。
- `preflight` 检查项列表。
- `build_context` 检查项列表。
...
- profile 引用 disabled plugin 时，Health Report 输出 `config.profile_refs: fail` 并返回 `3`。
- AI repair 启用但命令不可执行时，构建上下文校验阻断。
- 非 case-sensitive host path 作为构建工作树时，报告输出 `warn` 和迁移建议。
- Docker mount 范围包含 workspace root 时，构建上下文校验输出 `docker.mount_scope: fail`。
